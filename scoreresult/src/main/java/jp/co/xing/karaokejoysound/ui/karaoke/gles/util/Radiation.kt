...
class Radiation(context: Context) {
    ...
    private var infoBuffer = FloatBuffer.allocate(3)

    private var startTime = Date().time

    init {
        this.context = context
        val vertexShader: Int = loadShaderFromAssets(GLES32.GL_VERTEX_SHADER, "shaders/radiation-0.vertexshader")
        val fragmentShader: Int = loadShaderFromAssets(GLES32.GL_FRAGMENT_SHADER, "shaders/radiation-0.fragmentshader")
...
//        uboInfo1.unbind()
    ...
    fun draw(viewSize:Vec2 ) {
        ...
        infoBuffer.put(2,time.toFloat())//rectSize.x
