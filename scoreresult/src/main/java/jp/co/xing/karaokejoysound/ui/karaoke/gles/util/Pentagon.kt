...
class Pentagon(context: Context) {
    ...
    private var vertexCoords = floatArrayOf(
        ...
        +0.0f, +0.0f, 0.0f,      // #0: center
         0.0f, +1.0f, 1.0f,      // #1: upper center
        -1.0f, 0.0f, 2.0f,      // #2: upper left
        -1.0f, -1.0f, 3.0f,      // #3: Lower left
        +1.0f, -1.0f, 4.0f,      // #3: Lower right
        +1.0f, -0.0f, 5.0f,      // #3: upper right
    )
...
//        }
    ...
    private var listnerCalled = false

    init {
        this.context = context
        val vertexShader: Int = loadShaderFromAssets(GLES32.GL_VERTEX_SHADER, "shaders/pentagon-0.vertexshader")
        val fragmentShader: Int = loadShaderFromAssets(GLES32.GL_FRAGMENT_SHADER, "shaders/pentagon-0.fragmentshader")
...
//        infoBuffer.put(5,1.0f)
    ...
    fun draw(viewSize:Vec2, center:Vec2, length:Float, score:Array<Float>, period:Float,color:Color) {
        ...
        infoBuffer.put(5,time)//time
        infoBuffer.put(6,period)//period

        infoBuffer.put(7,score[0])//score
        infoBuffer.put(8,score[1])//score
        infoBuffer.put(9,score[2])//score
        infoBuffer.put(10,score[3])//score
        infoBuffer.put(11,score[4])//score
        ...
        if(listner!=null &&  started && time > period && !listnerCalled){
            listnerCalled=true
