...
class Star(val context: Context, val drawableId:Int) {
...
//    }
    ...
    private var vertInfo = FloatBuffer.allocate(7)
    ...
    init {
        ...
        val vertexShader: Int = loadShaderFromAssets(GLES32.GL_VERTEX_SHADER, "shaders/star.vertexshader")
        val fragmentShader: Int = loadShaderFromAssets(GLES32.GL_FRAGMENT_SHADER, "shaders/star.fragmentshader")
...
//    private val uvStride: Int = UV_COORDS_PER_VERTEX * 4 // 4 bytes per vertex
    ...
    fun draw(viewSize:Vec2,expRate:Float, angle:Float,time:Float) {
        draw(viewSize,viewSize,expRate,angle,time)
    }

    fun draw(viewSize:Vec2, texSize:Vec2,expRate:Float, angle:Float,time:Float) {
        ...
        vertInfo.put(4,expRate)
        vertInfo.put(5,angle)
        vertInfo.put(6,time)
